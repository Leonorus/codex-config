...
cat <<'EOF'
{
  "hookSpecificOutput": {
    ...
    "additionalContext": "Workflow: for substantial software/ops/debug/research/repo-maintenance, call mcp_workflow_start_task(prompt,cwd,repo) first; state/override bucket, load returned skills, follow contract/context/delegation/finish checklist. Use Workflow/Obsidian MCP before Obsidian claims and for workflow note writes. Finish non-trivia with mcp_workflow_finish_checklist. Fallback: codex-workflow. Prefer RTK-prefixed shell commands when practical, per RTK.md."
  }
}
EOF
