...
- For workflow note reads, writes, or updates, use `mcp__docker_gateway.obsidian_*` tools as the first note surface; avoid shell commands, raw filesystem access, redirection, heredocs, or Python scripts for vault notes unless those tools are unavailable, then state the fallback.
